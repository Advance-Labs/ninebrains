...
| `pr-hygiene` | DCO sign-offs, a Conventional Commit PR title, and the upstream-patch log: every changed file that existed at `dbf690c` must be named in the lines your PR adds to `docs/UPSTREAM-PATCHES.md`. A PR opened by Dependabot (`pr.user.login`/`type`, set by GitHub when the PR was created and unforgeable by any later push) skips only the upstream-patch log step, and only when the diff between base and head — computed locally with `git diff`, never read from the API — touches nothing but workflow/composite-action YAML, and every hunk is an equal-count, line-for-line swap of `uses:` pins where each pair keeps the same indentation, list marker and `owner/repo[/path]`: only the `@ref` and its trailing comment may change, in place (`tooling/scripts/bot-pr.mjs`). Commit metadata (`author.login`, `verified`, `committer`) is not trusted for this: GitHub signs a commit as `dependabot[bot]`/`web-flow` for anyone who pushes it through the Contents/Git Data API with the author email set to Dependabot's, so a forged commit can pass every per-commit check. The diff shape is the actual guarantee — a forged commit on the bot's branch can still only retarget an existing pin to a different ref, never move a step, swap an action, or add any other line — so it, not the commit metadata, decides the exemption. The DCO and title checks still run |
