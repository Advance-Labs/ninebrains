...
| `pr-hygiene` | DCO sign-offs, a Conventional Commit PR title, and the upstream-patch log: every changed file that existed at `dbf690c` must be named in the lines your PR adds to `docs/UPSTREAM-PATCHES.md`. A PR opened by a verified bot account (Dependabot) skips only the upstream-patch log step — a bot cannot write that row — checked by `tooling/scripts/bot-pr.mjs` against unforgeable GitHub fields (the PR author's account type, each commit's GitHub-verified signature and `web-flow` committer, single-parent commits); the DCO and title checks still run |
