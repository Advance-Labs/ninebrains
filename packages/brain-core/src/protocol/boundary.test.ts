...
describe('SEC-07 errors do not leak internals', () => {
  it('turns an unexpected failure into a bare INTERNAL and hands the error to main', () => {
    ...
    const secret = new Error('SQLITE_CORRUPT: SELECT * FROM jobs /Users/me/Library/ninebrains/brain.sqlite');
    store.listLanes = () => {
      throw secret;
    };
    const logged: unknown[] = [];
    const response = executeBrainRequest(makeBrain(store, { lanes: false }), hub, { v: 1, op: 'list_lanes', args: {} }, {
  ...
  it('validation and Brain errors carry a code and a message, never a stack', () => {
    ...
    for (const input of [{ v: 1, op: 'send_message', args: { to: 'x' } }, { v: 1, op: 'claim_job', args: { jobId: 'missing' } }]) {
      const response = executeBrainRequest(brain, laneA, input);
      expect(response.ok).toBe(false);
      expect(Object.keys((response as { error: object }).error).sort()).toEqual(['code', 'message']);
...
describe('SEC-09 inbox is structured', () => {
  it('every message carries from, and lane-written messages are untrusted', () => {
    ...
    executeBrainRequest(brain, laneA, { v: 1, op: 'send_message', args: { to: { kind: 'brain', id: 'main' }, body: injected } });
    executeBrainRequest(brain, hub, { v: 1, op: 'send_message', args: { to: { kind: 'lane', id: 'A' }, body: 'go' } });
    ...
    expect(laneInbox).toMatchObject({ ok: true, result: [{ from: { kind: 'brain', id: 'main' }, untrusted: false }] });
  });
});
