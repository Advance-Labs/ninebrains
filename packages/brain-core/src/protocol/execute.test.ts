...
beforeEach(() => {
  ...
  grants = {
    A: { identity: { role: 'lane', laneId: 'A', projectId: 'p1' }, projectId: 'p1', attachmentRoots: roots },
    B: { identity: { role: 'lane', laneId: 'B', projectId: 'p1' }, projectId: 'p1', attachmentRoots: roots },
    X: { identity: { role: 'lane', laneId: 'X', projectId: 'p2' }, projectId: 'p2', attachmentRoots: roots },
...
function call<O extends BrainOp>(who: keyof typeof grants, op: O, args: BrainOpInput<O>) {
  ...
  if (response.ok) {
    ...
    expect(check.success, `${op} result violates its schema: ${JSON.stringify(response.result)}`).toBe(true);
  }
  return response;
}
...
function result<O extends BrainOp>(who: keyof typeof grants, op: O, args: BrainOpInput<O>): any {
  ...
  if (!response.ok) throw new Error(`${op} failed: ${response.error.code} ${response.error.message}`);
...
describe('executeBrainRequest', () => {
  it('covers every operation, and each result matches its schema', () => {
    ...
    expect(track('A', 'complete_job', { jobId: first.id, summary: 'built', artifacts: ['out.png'] })).toMatchObject({
      state: 'verifying',
    });
  ...
  it('maps Brain errors to their codes', () => {
    ...
    expect(call('A', 'create_job', { title: 'sneaky' })).toMatchObject({ error: { code: 'FORBIDDEN' } });
    expect(call('hub', 'link_jobs', { from: job.id, to: job.id })).toMatchObject({ error: { code: 'CYCLE' } });
    expect(call('A', 'complete_job', { jobId: job.id, summary: 'x', artifacts: ['/etc/hosts'] })).toMatchObject({
      error: { code: 'INVALID', message: expect.stringContaining('outside') },
    });
  });

  it('requires a project when the grant has no default', () => {
    const response = executeBrainRequest(brain, { ...grants.hub, projectId: null }, { v: 1, op: 'create_job', args: { title: 'T' } });
