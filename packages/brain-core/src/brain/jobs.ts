...
export function settle(ctx: BrainContext, tx: Tx, job: Job): Job {
  ...
  if (job.state === 'proposed' && dependenciesDone(ctx, job.id))
    return transition(ctx, tx, job, 'ready');
  if (job.state === 'ready' && !dependenciesDone(ctx, job.id))
    return transition(ctx, tx, job, 'proposed');
  return job;
}
...
export function createJob(
  ...
  for (const dep of new Set(input.dependsOn ?? [])) {
    ...
    if (source.projectId !== job.projectId)
      throw new InvalidInputError(`dependency ${dep} is in another project`);
...
export function linkJobs(
  ...
  if (source.projectId !== target.projectId)
    throw new InvalidInputError('cannot link jobs across projects');
...
export function assignJob(
  ...
  if (lane.projectId !== job.projectId) {
    throw new ForbiddenError(
      `lane ${laneId} belongs to project ${lane.projectId}, job is in ${job.projectId}`
    );
  }
  if (job.state !== 'ready') {
    throw new IllegalTransitionError(
      ...
      job.state,
      'claimed',
      'only ready jobs can be assigned'
    );
  }
...
export function completeJob(
  ...
  if (artifacts.length > LIMITS.artifacts)
    throw new InvalidInputError(`at most ${LIMITS.artifacts} artifacts`);
...
export function requeueJob(ctx: BrainContext, tx: Tx, identity: Identity, jobId: JobId): Job {
  ...
  if (job.state !== 'blocked' && job.state !== 'failed') {
    throw new IllegalTransitionError(
      ...
      job.state,
      'ready',
      'only blocked or failed jobs can be requeued'
    );
  }
  const target = dependenciesDone(ctx, job.id) ? 'ready' : 'proposed';
  if (job.laneId) setLaneJob(ctx, tx, job.laneId, null);
  return transition(ctx, tx, job, target, {
    attempts: 0,
    laneId: null,
    reason: null,
    result: null,
