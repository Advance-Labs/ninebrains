...
describe('SEC-08 caller cannot drop gates', () => {
  ...
  it('callers can only add: requested gates join the floor, floor first, deduplicated', () => {
    ...
    const job = brain.createJob(BRAIN, {
      projectId: 'strict',
      title: 't',
      gateSpec: { gates: ['screenshot', 'tests'] },
    });
  ...
  it('only a project whose floor is empty (rigor 0) can have an unverified job', () => {
    ...
    expect(
      gatesOf(
        brain,
        brain.createJob(BRAIN, { projectId: 'lax', title: 't', gateSpec: { gates: ['tests'] } }).id
      )
    ).toEqual({
      gates: ['tests'],
    });
  ...
  it('defaults to no floor when the app supplies no resolver', () => {
    ...
    expect(
      brain.createJob(BRAIN, { projectId: 'p1', title: 't', gateSpec: { gates: [] } }).gateSpec
    ).toEqual({ gates: [] });
  });
});
