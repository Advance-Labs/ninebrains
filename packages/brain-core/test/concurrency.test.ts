...
describe('multi-process claims on one SQLite file', () => {
  it('exactly one of several processes racing for the same task wins', async () => {
    const dir = tempDir();
    const dbPath = path.join(dir, 'brain.sqlite');
    const setup = new Brain({ store: SqliteBrainStore.open(dbPath) });
    const task = setup.createTask(BRAIN, { projectId: 'p1', title: 'contested' });
    setup.close();

    const lanes = ['L1', 'L2', 'L3', 'L4', 'L5', 'L6'];
    const results = await race(path.join(dir, 'barrier'), lanes, (lane) => [dbPath, lane, path.join(dir, 'barrier'), 'race', task.id]);
    ...
    expect(check.getTask(BRAIN, task.id)).toMatchObject({ state: 'claimed', laneId: winners[0]!.laneId });
    check.close();
  }, 60_000);

  it('several processes draining a queue claim every task exactly once', async () => {
    const dir = tempDir();
    const dbPath = path.join(dir, 'brain.sqlite');
    const setup = new Brain({ store: SqliteBrainStore.open(dbPath) });
    const ids = Array.from({ length: 40 }, (_, i) => setup.createTask(BRAIN, { projectId: 'p1', title: `t${i}` }).id);
    setup.close();
    ...
    for (const result of results) {
      for (const id of result.claimed as string[]) {
        expect(claimedBy.has(id), `task ${id} claimed twice`).toBe(false);
    ...
    const check = new Brain({ store: SqliteBrainStore.open(dbPath) });
    for (const t of check.listTasks(BRAIN)) {
    ...
    const claims = check.readEvents(0, 10_000).filter((e) => e.type === 'taskChanged' && e.payload.task.state === 'claimed');
