...
Loop: read_inbox -> claim_job -> do the work -> complete_job with a summary and evidence. If stuck, call block_job with what would unblock you.
...
/** Builds an MCP server for one role over a backend. Transport-agnostic: the bin connects stdio. */
export function createBrainMcpServer(options: { backend: BrainBackend; role: Role }): McpServer {
  const server = new McpServer(
    { name: SERVER_NAME, version: SERVER_VERSION },
    { instructions: options.role === 'brain' ? BRAIN_INSTRUCTIONS : LANE_INSTRUCTIONS }
  );
  registerTools(server, options.backend, options.role);
  return server;
}
