...
whose diff (computed locally with git) touches only workflow/composite-action YAML with every
changed line a bare `uses:` pin bump — commit-level metadata is deliberately not trusted, since
GitHub signs a commit identically for anyone who pushes it through the Contents/Git Data API with
the author email set to Dependabot's.
