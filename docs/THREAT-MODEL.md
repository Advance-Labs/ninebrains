...
| T44 | The SEC-30 reap that runs when a run's process closes (`run-supervisor.ts` ~456) SIGKILLs the run's process group to catch anything the agent left behind (dev servers, watchers). Under load, the OS can recycle a pid — and so the process-group id, which is the leader's own pid — between the leader exiting and this reap signal, so `process.kill(-pid, …)` (`signalGroup`, `process-group.ts`) can throw `EPERM` against a group we never started. That throw happened *before* `resolve(result)` in the `close` handler, so the run's promise never settled: no hang bound, no result, ever, for that run. `terminateGroup`'s own `signalGroup` calls (STOP's `killAll`, the wall-clock SIGKILL escalation) had the same "throw blocks settle" shape, called fire-and-forget from a timer or `void`, which would have surfaced as an unhandled rejection instead | M | H | **Fixed 2026-09-14:** `signalGroup` treats `EPERM` the same as `ESRCH` — the group is gone from our side either way, so it doesn't throw. `run-supervisor.ts`'s reap call is also wrapped directly, as a backstop for anything `signalGroup` doesn't already recognize: the failure is recorded to the run's transcript and its `errors`, but `resolve(result)` always runs. `terminateGroup`'s three `signalGroup` calls (`process-group.ts`) now go through `trySignalGroup`, which logs and never throws, so a STOP or a wall-clock timeout can't leave an unhandled rejection either. Tests (`run-supervisor.test.ts`, `SEC-30 kill switch`): a stubbed `process.kill` throws `EPERM` once on the process-group form of the call only (every other call, including the tests' own liveness checks, reaches the real syscall, so nothing is left running afterward) — one run still resolves `completed` with no reported error (EPERM is tolerated, like ESRCH always was), `killAll` still kills every run and stays latched, and a *non*-tolerated code (`EIO`) still resolves the run and is reported in its `errors`. **Residual risk, stated plainly:** this fix stops the hang; it does not close the pid-reuse race itself (R14: "killing a reused pid is worse"). If the recycled id has, by the time we signal it, become the process-group id of an unrelated group started by the *same* user, `kill(-pgid)` succeeds and signals that group — Ninebrains cannot tell the difference from a pid and a signal result alone. Job-object/cgroup-scoped tracking (R14's own open mitigation) is what would actually close this, not this fix | R14, SEC-30 |
