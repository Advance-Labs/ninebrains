...
| T46 | The new `agentCliStatus` panel (`docs/plans/2026-09-15-routing-usability.md` item 1) echoes a role's assigned profile label and tier, and a blocked reviewer's reason string, to the renderer for the first time — new information-disclosure surface, even though it's the user's own configuration | L | L | It reflects the same profile list already visible via `listProfiles`, and reports only which existing SEC-42-checked decision applies, never a key. Mitigated by reuse, not new code: `installed`/`path` come from the same `hostDependencies.resolver` call already made for the reviewer's `installed` set (D5: no new probing, no credential file read). **Corrected 2026-09-15 (independent security review, same day as this row was added):** the reviewer row initially called `resolveRoute('reviewer', { profiles })` with no explicit id — the tier-scan branch no real launch path uses — so it could show a profile the reviewer would never actually run under, or `subscription` for a reviewer really pinned to one, and could never render `blocked` at all, the one state a user most needs to see. Fixed: the reviewer row now goes through `reviewerRoleMode`, which reads the exact pin `routeReviewer` reads (the one closure `create-ninebrains-services.ts` passes to both) and checks it with `reviewerProfileStatus` — the same predicate `prepareReviewerRoute` checks before it reveals a key, factored out so the two cannot drift into different definitions of "blocked" the way the original two paths did. `reviewerProfileStatus` never calls `deps.keys.reveal`; it checks the `hasKey` flag already on the (unrevealed) profile record, so the panel keeps its no-reveal property (tests spy on `keys.reveal` and assert zero calls across every `agentCliStatus` case). Worker and subagent are named as an approximation, in the wire contract's doc comment and in the panel's own UI copy: they still use `resolveRoute` with no explicit id, which is the tier default a lane gets when it names no profile of its own, not a guarantee for every lane's actual `prepareLaunch(lane)` result. Tests: `routing-service.test.ts`'s `agentCliStatus` suite (reviewer unpinned/pinned/pinned-but-disabled/profiles-off, and the `keys.reveal` zero-calls assertion), `policy.test.ts`'s `reviewerProfileStatus` suite | SEC-42, SEC-44 |
| T47 | `MODEL_PROFILES_ENABLED` (fork-flags.ts) stops being a build-time hold-back: Lever B (model profiles) now compiles into and ships in every build, release included, and whether it is *active* moves to a user-controlled app setting (`ninebrains.routing.profilesEnabled`, default `false`). This widens the trust boundary two ways: (1) a release-build user, not only a dev build, can now reach the profile CRUD, keychain writes and profile-routed launches SEC-39 through SEC-45 already govern; (2) `RoutingServiceDeps.enabled` and the lane-service fast-fail (`LaneService`'s `profilesEnabled()`) both moved from a value captured once at boot to a live read on every call, so a toggle takes effect mid-session — a new place a stale-vs-fresh read could disagree and let something slip through | M | M | Every SEC-39 through SEC-45 guarantee holds unchanged regardless of who flips the switch: nothing about the keychain (SEC-40), the vendor allowlist (SEC-44), the reviewer pin (SEC-42), egress (SEC-45) or the subscription-only launch env (SEC-39) reads or trusts this setting directly — they all still gate on the profile itself once `enabled()` is true. `enabled()` defaults to `false` and is read fresh (`await deps.enabled()`) at the top of every `RoutingService` method and inside `routeReviewer`'s `reviewerProfileId` closure, so off-by-default and "off ignores stored profiles and a stored pin" hold for every call, not just the first one after boot; a profile row or a reviewer pin left in storage while off is never read, only ignored (`routing-service.test.ts`'s `T47: enabled() is read live` suite proves a flip from on to off mid-session stops routing to an already-saved profile without deleting it). The setting itself is reachable only through the app-settings wire controller (`manifests/node/controllers.ts`), which — like the gates project-prefs settings SEC-08 already covers — has no counterpart in brain-core's `BrainOp` vocabulary (`LANE_OPS`/`BRAIN_OPS`/`SESSION_OPS`), so no lane or Brain token can flip it, structurally, not by a runtime check (`contributions/settings.test.ts`'s `SEC-08` suite). The lane-service fast-fail is UX only, not the security boundary — a stale "allowed" read there still hits the authoritative, always-live `deps.enabled()` check in `RoutingService.prepareLaunch` at actual launch time, which throws rather than falling back to the subscription. Turning the setting on by itself changes nothing observable: with no profile configured, `listProfiles`/`agentCliStatus`/`prepareLaunch` all still report the subscription, proven by the existing `'both CLIs installed, no profiles: every role...is the subscription'` case now running with `profilesEnabled` newly `true`. The UI states plainly, once, next to the toggle (not a repeated dialog) that a key added afterward is a real, billed API call | SEC-08, SEC-39, SEC-40, SEC-42, SEC-43, SEC-44, SEC-45 |
...
- **SEC-47 Cowork socket and token boundary.** The Cowork server accepts clients only through a
  Unix socket in a server-owned directory that is not group or world writable. The socket is
  group-accessible; state and token files remain owner-only. Joining requires a capability token
  compared in constant time. Only joined clients receive document updates.
- **SEC-48 Cowork file confinement.** Document paths must be relative to the configured worktree.
  The server canonicalizes the path, rejects symlink escapes and non-text files, and checks the
  path and opened file identity again at Save. Persisted document state stays outside the worktree.
- **SEC-49 Cowork convergence and disk protection.** A document update is persisted before it is
  acknowledged. The server limits peers, documents, frames, and document size. Save refuses a disk
  version changed outside Cowork. Ordinary external writers do not honor its lock, so a write in
  the final check/write interval remains a race to address separately.
...
| SEC-47 | Partial | `apps/cowork-server/src/server.test.ts` covers rejected tokens and socket permissions; two-account Linux validation is pending |
| SEC-48 | Partial | `apps/cowork-server/src/server.test.ts` covers traversal and symlink escape; two-account Linux validation is pending |
| SEC-49 | Partial | `apps/cowork-server/src/server.test.ts` covers convergence, restart, and external disk changes; the external-writer race remains |
