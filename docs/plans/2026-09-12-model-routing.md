...
New threats: **T38** a subscription token sent to a third-party base URL (SEC-39, SEC-41). **T39** a
lane's Bash reads the profile key from its env and spends directly: the guide tells users to set a
vendor-side spend limit per key; accepted risk R19. **T40** a reviewer silently downgraded
(SEC-42). **T41** a mispriced or unpriced model makes the budget meaningless (SEC-43).
