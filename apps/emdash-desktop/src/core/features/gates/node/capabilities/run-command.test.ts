...
describe('SEC-20 tests gate is sandboxed', () => {
  it('env output has no token, no NINEBRAINS_*, no provider secrets', async () => {
    ...
    for (const needle of ['NINEBRAINS', 'nb-live-token', 'sk-ant-', 'ghp_', 'CLAUDE_CONFIG_DIR', 'ANTHROPIC']) {
      expect(r.stdout).not.toContain(needle);
    }
    expect(r.stdout).toMatch(/^PATH=/m);
  });

  it('kills a forked grandchild that ignores SIGTERM within 5 s of abort', { timeout: 20_000 }, async () => {
    ...
    for (let i = 0; i < 100 && !(existsSync(pidFile) && readFileSync(pidFile, 'utf8').trim()); i++) {
  ...
  it('caps output, keeping the tail', async () => {
    const small = createRunCommand({ allowedRoots: () => [worktrees], ninebrainsDataDir: ninebrains, maxOutputBytes: 1000 });
    const r = await small('i=0; while [ $i -lt 2000 ]; do echo line-$i; i=$((i+1)); done', { cwd: laneA, signal: signal() });
  ...
  it('runs argv without a shell', async () => {
    const r = await runCommand('printf', { argv: ['%s|', 'a b', '$HOME', ';id'], cwd: laneA, signal: signal() });
  ...
  it('refuses a cwd outside the worktree roots', async () => {
    await expect(runCommand('true', { cwd: outside, signal: signal() })).rejects.toThrow(/not inside/);
  });
  ...
  it('escapes profile paths', () => {
    ...
    expect(profile).toContain('(require-any (subpath "/wt/a") (subpath "/tmp/nb-tests-x") (subpath "/dev"))');
  });
...
describe.skipIf(process.platform !== 'darwin' || !existsSync('/usr/bin/sandbox-exec'))(
  ...
  () => {
    it('denies reading Ninebrains data and sibling worktrees', async () => {
      const db = await runCommand(`cat '${join(ninebrains, 'brain.sqlite')}'`, { cwd: laneA, signal: signal() });
      expect(db.exitCode).not.toBe(0);
      expect(db.stdout).not.toContain('TOP-SECRET-DB');
      const sibling = await runCommand(`cat '${join(laneB, 'mcp.json')}'`, { cwd: laneA, signal: signal() });
    ...
    it('denies writes outside the worktree and allows them inside', async () => {
      const escaped = await runCommand(`echo x > '${join(outside, 'w.txt')}'`, { cwd: laneA, signal: signal() });
      expect(escaped.exitCode).not.toBe(0);
      expect(existsSync(join(outside, 'w.txt'))).toBe(false);
      const local = await runCommand('echo ok > ok.txt && cat ok.txt', { cwd: laneA, signal: signal() });
    ...
    it('gives each command a private, writable TMPDIR that is removed afterwards', async () => {
      const r = await runCommand('echo t > "$TMPDIR/t.txt" && cat "$TMPDIR/t.txt" && echo "$TMPDIR"', {
        cwd: laneA,
        signal: signal(),
      });
      expect(r.exitCode).toBe(0);
