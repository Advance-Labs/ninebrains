...
dispatcher, the verification hand-off, Brain sessions and the global STOP.
Glossary: a **Job** is a Brain work item; a **lane** is one agent in a worktree
(`features/lanes`).

**This slice is headless (M5 / SEC-47).** The Brain drawer, the titlebar controls,
the lane run-mode toggle, the Settings card, the Arena section and the
`brain.stopAll` command are gone. The operator drives the Brain with `brain`, the
CLI in `packages/brain-cli`, over the same hardened endpoint the lanes use — see
that package's README. The wire contract's live models stay: they are how the
node side is tested and how a future UI would attach again.
...
| `node/endpoint.ts` | `startBrainHttpServer` on 127.0.0.1 plus a ledger: one token per launch key (`lane:`, `brain:`, `run:`, `user`). `attachHost` binds the host ops late; `publishCliHandshake` mints the operator's token and writes `<userData>/ninebrains/brain-cli.json` 0600 |
| `node/cli-host.ts` | `BrainHostOps` over `BrainService`: the controls the drawer used to own, and the one place the app's `Result<T, BrainError>` meets brain-core's error codes |
...
| `contributions/mementos.ts` | The `brain.sessions` versioned schema. The slice's only contribution left: it is persistence, not UI |
...
`brain status` reports dispatcher state and `brain jobs` every open job, across
projects. This works because `BrainService` is a single app-wide instance
(constructed once in `create-ninebrains-services.ts`); `projectId` is a scoping
parameter on individual calls, not a sign of per-project instantiation. `overview`
(`unread` / `sessions` / `dispatcher`) was already unkeyed and already spanned
every project on the backend.

`BrainViews.refresh()` uses `brain.listJobs(APP_IDENTITY, { states: OPEN_JOB_STATES,
limit: 500 })`. `states` matters: without it the query sorts by creation time
ascending with no way to exclude finished work, so a filtered-only-by-limit query
eventually fills up with the oldest `done` jobs (`done` entries are never archived)
and the cross-project view silently goes stale forever past a few hundred lifetime
jobs. `allJobs` is its own live model, not a fourth field on `overview` — see
`api/contract.ts`'s comment for why.

A user *token* is not per project; an agent's Brain token still is (see Accepted
risks below). That is a narrower constraint than instance scope, easy to conflate
with it.
...
| SEC-30 | STOP: latch, `killAll`, stop dispatched attended lanes, answer within 5 s. The surfaces are now the app menu and tray (main) and `brain stop` | `stop.test.ts` › `SEC-30 global STOP` |
| SEC-47 | The user role: `USER_OPS`, host ops through `ExecuteOptions.host`, the 0600 handshake | brain-core `user-role.test.ts`, `handshake.test.ts`; `brain-cli/test/cli.test.ts` |
...
8. **Lane run mode is persisted with the lane** (`runMode` on the lane config;
   ...
   `brain mode <lane> <attended|unattended>` sets it, and `brain status` reports
   every lane's mode with the unattended budgets.
...
11. **STOP from main.** `onStopChange` lets `main/host/ninebrains/agent-stop-controls.ts`
   ...
   directly, never through the renderer. That file also owns the
   `Mod+Shift+Backspace` chord now, since the renderer command is gone (M5).
13. **The CLI is a role, not a runtime.** `brain` attaches to the app's endpoint
   rather than opening the DB, so SEC-01 holds unchanged and there is one
   implementation of every control. A Brain that runs without the app is
   `docs/plans/brain-headless-cli.md`, not this.
